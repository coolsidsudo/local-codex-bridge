...
Local Codex Bridge 是一个基于项目配置的本地 MCP 服务器。它允许 ChatGPT 启动并检查你自己电脑上的 **本地 Codex CLI** 任务。

它适合这样的工作流：你不想使用云端 Codex，而是希望 Codex 在本地仓库中运行，使用本地 git、真实本地文件系统，以及由你自己控制的 Codex 模型，例如本地 Codex CLI 支持的 `gpt-5.5`。
...
ChatGPT
  ...
  -> 检查项目配置、git status、HEAD 和 remotes
  -> 启动有边界的本地 Codex 任务
  -> 读取任务日志、diff 和验证输出
  -> 审查结果
...
v0 不暴露任意 shell 执行。验证命令必须在每个项目 profile 中显式 allowlist。
...
8. MCP server URL 填入当前 ngrok URL 加 `/mcp`：
...
2. 启动 ngrok http 8765。
3. 如果 ngrok URL 改变，刷新 ChatGPT connector URL。
...
10. 在接受任何结果前，审查 stdout/stderr、git diff 和 verification。
```
...
这个 bridge 可以让本地 Codex 修改已配置仓库中的文件。请把它当作强大的本地自动化工具。
...
- 验证命令必须 allowlist。
- 在 commit、push 或 merge 前审查 diff。
