...
`run_verification_bundle` accepts configured verification keys only, rejects unknown or invalid keys before running anything, runs the fixed allowlisted argv arrays sequentially with `shell=False`, and returns bounded per-command stdout/stderr evidence. It does not accept arbitrary command argv at runtime, and the bundle orchestration itself adds no Git/GitHub/PR/tag/release mutation authority. Actual side effects are those of the configured allowlisted command, so operators should configure verification keys as read-only commands if they want read-only verification semantics.

The optional `start_codex_task` review contract is prompt guidance only. It asks Codex to return concise implementation summaries instead of full diffs or full file contents, but it is not a security boundary and must not be trusted for enforcement. ChatGPT and the human reviewer should inspect actual repository state, verification evidence, and readiness evidence through Local Codex Bridge tools before any explicit commit, push, PR, merge, sync, tag, or release operation.
